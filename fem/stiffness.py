"""
Matriz de rigidez del elemento (version legible, elemento a elemento).
kₑ = ∫∫ Bᵀ D B |det(J)| t dξ dη

`element_stiffness` retorna ke junto con `gauss_data`: TODOS los intermedios
por punto de Gauss (J, det J, inv J, dN_nat, dN_phys, B y la contribucion a
ke). La consumen los modulos educativos (M5, M7), la memoria de calculo
(paso a paso del elemento estrella) y `tests/test_solver_regression.py`,
para el que es el oraculo.

El solve productivo NO pasa por aqui: `fem/assembly.py` usa la version
vectorizada por lotes de `fem/batch.py` (misma formulacion, todos los
elementos a la vez), que debe coincidir con esta a 1e-9.
"""
