"""
Biblioteca de modelos de ejemplo de EduFEM (capa de PRODUCTO).

Estos loaders alimentan el menú **Ayuda ▸ Cargar Ejemplo** (3 casos × Q4/Q9),
una feature visible al alumno — NO son fixtures de test. Por eso viven en
`models/` (producto) y no en `tests/`: el ejecutable PyInstaller debe poder
cargarlos aunque `tests/` no se empaquete. Los scripts de validación
(`tests/vv_*`) también los consumen (tests → producto es la dirección correcta).

Se movieron desde `tests/example_data.py` en 2026-05; ese archivo quedó como
shim de re-export para no romper imports existentes.
