...
| `mod03` | ③ Matriz constitutiva D | **Overlay** | **Espectro de Poisson** (slider horizontal con anclas de material clickeables) + toggle F/V + warning de locking. |
| `mod04` | ④ Matriz B | **Overlay** | **Cuadrado natural matplotlib estilo M2, CENTRADO** (click fija ξ,η — bidireccional con el canvas físico) + glow pulsante en Gauss + snap a Gauss + toggle F/V: **Fórmula** = relación vectorial `[∂Nᵢ/∂x;∂Nᵢ/∂y]=J⁻¹[∂Nᵢ/∂ξ;∂Nᵢ/∂η]` + B simbólica `\dfrac`; **Valores** = cadena `∂N_ξη·J⁻¹→∂N_xy→B` (replicable en Excel). |
...
| M3 | proc | **Overlay** | **Espectro de Poisson** (slider horizontal `[0, 0.5]` que fusiona ex-dial + ex-regla de materiales + ex-Reset + dropdown de presets en un único control) + toggle Fórmula↔Valores + warning de locking volumétrico cuando ν → 0.5 en DP. Ver "Refactor 2026-05: M3 — espectro de Poisson" abajo. |
| M4 | proc | **Overlay** | **Cuadrado natural matplotlib estilo M2, CENTRADO** (click fija ξ,η; bidireccional con el canvas) + glow pulsante (~30 fps) en puntos Gauss + toggle Fórmula↔Valores (B simbólica `\dfrac` + relación J⁻¹). |
...
- **Notación canónica de matrices de derivadas (unificada 2026-05, M4 como referencia)** — la misma entidad lleva el MISMO símbolo en todos los archivos user-facing (overlays + memoria PDF + theory hub). Verificada por workflow de inventario sobre M1/M2/M4/M5/memoria/theory_hub:
...
**Paso a paso = reemplazo matricial, consistente con M2/M4** (2026-05): el desarrollo elemental NO muestra solo la matriz resultado — muestra la **cadena de sustitución** como los módulos educativos. `_build_pg_chain` recalcula por punto de Gauss con las **mismas funciones del solver** (`fem.shape_functions.get_shape_functions` → `dN_func`, `fem.jacobian.compute_jacobian`/`compute_dN_physical`, `fem.b_matrix.compute_b_matrix`) → consistencia bit-a-bit con M2/M4. Se exhibe: `Xₑ` (matriz n×2 en Geometría); **Jacobiano** `J = ∂N·Xₑ` con el producto explícito `[∂N][Xₑ]=[J]` (Q4) + `det J = J₁₁J₂₂−J₁₂J₂₁ = (a)(b)−(c)(d) = valor` (reemplazo numérico); **B** `J⁻¹ → ∂N/∂x = J⁻¹·∂N → B`. **Q9 (wide, n>4)**: el triple producto se parte (∂N aparte + J), la tabla N va en `\scriptsize` (13 columnas), y `B` (3×18) va en `\begin{landscape}\tiny` (como kₑ 18×18). `_mostrar_jacobiano_sub` / `_mostrar_b_sub` reciben `wide=n_nodes>4`. **No reintroducir** la versión que mostraba solo `J`/`B` resultado sin la sustitución (rompía la consistencia con los módulos).
