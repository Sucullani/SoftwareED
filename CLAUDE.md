...
|---|---|---|
| DOF (Degree of Freedom) | **GDL** (Grado de Libertad) | Singular "GDL", plural "GDLs". Aplica al chip `Q4 · 8 GDL` / `Q9 · 18 GDL`, a los mensajes del validador de salud, al PDF de memoria de cálculo, etc. |
...
- ex-`mod02_b_matrix.py` → `mod04_b_matrix.py` → (2026-05) `mod03_b_matrix.py`
...
> **Nota de numeración (swap B↔D 2026-05)**: en los bullets históricos de abajo, las referencias `M4` a la **matriz B** corresponden hoy a **M3** (`mod03_b_matrix.py`), y `M3` a la **matriz constitutiva D** corresponde hoy a **M4** (`mod04_constitutive.py`). El contenido (diseño de cada overlay) sigue vigente; solo cambió el número/orden.
...
- `margin_formula(latex, phase)` — fórmula clave al margen (`\marginpar`, `\scriptsize`, `marginparwidth=2cm`). Mantener las fórmulas **compactas** (sin `\frac` ancho) o desbordan el margen 2.2cm.
...
API pública `solve_system(project) → dict` queda fija para no obligar a tocar `post_tab`, `pdf_report`, M5.

Estado actual (lo de "alta" YA está implementado — no es pendiente):

1. **K sparse** (alta) — ✅ **Implementado**. `fem/assembly.assemble_global_system` acumula tripletes COO por elemento (scatter vectorizado con `np.repeat`/`np.tile`/`ke.ravel()`, evitando `meshgrid`) y construye `coo_matrix((data,(rows,cols))).tocsr()` ([assembly.py:290-291](fem/assembly.py#L290-L291)) — K **nunca** se materializa densa en producción (memoria O(nnz)). La rama densa solo sobrevive como fallback de los tests de backward-compat y para el `K.toarray()` del módulo educativo M5.
2. **Solver SPD sparse** (alta) — ✅ **Implementado**. `fem/solver._solve_reduced` resuelve con `scipy.sparse.linalg.spsolve` (SuperLU, el default de scipy) sobre K_red en CSR ([solver.py:93](fem/solver.py#L93)); tolera K densa por backward-compat (`.tocsr()` con fallback). `sksparse.cholmod.cholesky` (Cholesky para SPD, con import diferido tipo `_require_cholmod()`) y UMFPACK (vía `scikit-umfpack`) siguen siendo opt-in **futuros** — no instalados.
3. **RCM** (media) — ✅ **Implementado** tras el flag `SOLVER_USE_RCM` en `config/settings.py` (DEFAULT `False`): `scipy.sparse.csgraph.reverse_cuthill_mckee` antes de factorizar (2–5× menos fill-in en mallas estructuradas); `fem/solver._solve_reduced` permuta K_red, resuelve y despermuta. Con el flag en `False` el output es bit-a-bit idéntico al solver clásico; con `True` coincide dentro de 1e-13. Activar para comparar en benchmarks (scripts V&V).
4. **Ensamblaje vectorizado / batch** (media) — ⛔ **NO implementado** (y probablemente innecesario en este proyecto). El loop por elemento de `assemble_global_system` YA está acelerado: kernel `element_stiffness_fast` con `@njit` (no-op sin numba), scatter COO vectorizado, y precómputo de `dN`/`N`/puntos Gauss una sola vez por tipo de elemento. Lo único pendiente sería **fusionar todos los elementos en un solo `np.einsum("egki,kl,eglj,eg,g->eij", ...)`** batch. Solo valdría la pena con **miles** de elementos donde el overhead del loop Python domine aun con numba; para mallas educativas no se nota. **Mantener la versión escalar/loop como referencia pedagógica** (M2/M3/M5). `VECTORIZED_THRESHOLD` **no existe** todavía — definirlo solo si alguna vez se agrega el path batch.
...
- **Compatibilidad con M2/M3/M5**: si añadís variante optimizada, exponer también la legible vía `return_intermediate=True` o `fem/educational.py` paralelo. **No** sacrificar claridad pedagógica por velocidad.
...
- **Preservar la versión legible** cuando el código tiene valor pedagógico (M1..M7, `fem/` referencia para M2/M3/M5) — exponer la variante optimizada en paralelo, no reemplazar.
