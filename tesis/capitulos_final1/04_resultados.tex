...
\hline
\end{tabular}
{\singlespacing\notaapa{Los errores son absolutos y, como los datos de la configuración base, carecen de unidades; los relativos de la misma configuración están en la \autoref{tab:mms-rel}. Cada tasa se calcula con los errores sin redondear y puede diferir en una centésima de la que se obtiene con los valores impresos.}}
\end{table}
...
$|u_y|$ en A & $2{,}03101$ & ---         & $2{,}02310$ & ---    & 0,39 \\
$|u_y|$ en B & $1{,}09871$ & ---         & $1{,}09260$ & ---    & 0,56 \\
$|u_y|$ en C & $1{,}45216$ & ---         & $1{,}44530$ & ---    & 0,47 \\
\hline
\end{tabular}
\end{table}
...
\caption[Las tres fases del análisis sobre el mismo lienzo y el mismo modelo.]{Las tres fases del análisis sobre el mismo lienzo y el mismo modelo (ejemplo canónico: nueve nodos, cuatro elementos Q4, carga en el nodo 7). De arriba abajo: pre-proceso, con el modo de dibujo agregando un elemento sobre la malla existente; proceso, con el módulo M3 desplegado sobre el elemento seleccionado y los índices de grado de libertad junto a cada nodo; post-proceso, con el contorno de von Mises sobre la deformada, las reacciones en los apoyos y el panel de detalles del nodo 7 con su círculo de Mohr. Capturas de EduFEM; a tamaño completo, en el \autoref{anexo:manual}.}
...
Construcción paso a paso \autocite[pp.~1010-1011, 1023]{bishay2020teaching} & Un módulo por etapa, en el orden del método; memoria de cálculo sobre el modelo propio, integrada en un documento, con cada etapa rotulada & \autoref{tab:modulos}; \autoref{sec:memoria-io}; \autoref{anexo:memoria} \\
...
Esta sección interpreta lo que respaldan los tres casos de estudio y el límite que encontró la propia batería de verificación. Cierra con la posición de EduFEM frente a los recursos existentes y con las limitaciones que el desarrollo dejó a la vista.
...
Las componentes secundarias de tensión, de magnitud mucho menor y recuperadas con un orden de convergencia inferior, requieren mallas más finas para alcanzar el mismo nivel relativo que $\sigma_x$ (\autoref{sec:timoshenko}).
...
Error del desplazamiento con Q9 y 8 elementos por lado (Cook), en valor absoluto & $< 1{,}5\,\%$ & $-0{,}144\,\%$ & \autoref{tab:cook} & Se cumple \\
...
\end{longtable}}
{\singlespacing\notaapa{Los seis criterios de la cláusula (a) son de exhaustividad: su umbral es el total del universo, y por eso coincide con lo observado cuando se cumplen. A cada uno lo refutaría un solo caso en contra, el que precisa la columna «Lo refutaría» de la \autoref{tab:criterios}.}}
...
Queda así respondido el problema científico en el alcance declarado, que se repite aquí: lo que se midió son propiedades del software y de sus resultados, no el uso de la herramienta en la Carrera. Frente al software de análisis que devuelve la respuesta como caja negra, un software educativo con tres rasgos hace que el análisis por el MEF en elasticidad plana resulte trazable y verificable. Construye el procedimiento de cálculo con las mismas funciones que exponen los módulos y la memoria, muestra la respuesta junto con su procedimiento y sus medios de comprobación, y somete su motor a verificación y validación con criterios fijados de antemano. La \autoref{sec:leccion-extrapolacion} muestra que esa relación no es una tautología: la trazabilidad hizo visible un defecto de exactitud que las métricas agregadas no delataban, y ese defecto pudo corregirse.
