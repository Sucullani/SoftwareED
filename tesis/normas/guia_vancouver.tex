...
Este documento no forma parte de la tesis.
\end{minipage}
\end{center}
...
Las 22 fuentes de \texttt{referencias.bib} se reparten en cuatro tipos: 12 libros, 7 artículos
de revista, 2 informes técnicos y 1 manual de organización. Una ficha por tipo.

% ---------------------------------------------------------------------
\begin{ficha}{A \textperiodcentered\ LIBRO COMPLETO \hfill \normalfont\texttt{@book} \textperiodcentered\ 12 entradas}
\rot{plantilla}
...
\rot{ejemplo --- ejemplar sin pie de imprenta}
\refv{Álvarez de Zayas C. Metodología de la investigación científica. [lugar desconocido]:
...
\rot{ejemplo --- sin número de fascículo}
\refv{Pérez-Santiago R, Campos E. FEM education in undergraduate studies: industry-informed
research. Comput Appl Eng Educ. 2023;31:1159-73. doi:10.1002/cae.22627}
\anot{El fascículo se omite cuando el ejemplar no lo consigna. No se inventa.}
...
CSiBridge. Rev. 18. \textcolor{ojo}{[lugar]}: Computers \& Structures, Inc.; 2017.}
\anot{\textcolor{ojo}{Falta el lugar de edición}: hay que leerlo de la portada o de la página
de créditos del PDF del manual y agregarlo al \texttt{.bib}.}
...
@manual{csi2017sap2000,
  ...
  title   = {{CSI} Analysis Reference Manual for {SAP2000}, {ETABS},
             {SAFE} and {CSiBridge}},
  version = {Rev. 18},
  location = {...},          % <-- leer de la portada del manual
  year    = {2017},
}
\end{Verbatim}
\anot{Las llaves dobles \texttt{\{\{...\}\}} son obligatorias: sin ellas biber lee «Inc.» como
apellido y «Computers \& Structures» como nombre de pila. Y no pongas \texttt{author} y
\texttt{organization} a la vez: el estilo imprime los dos y la editorial sale duplicada.}
\end{ficha}
...
\textperiodcentered\ Strang \textperiodcentered\ Oberkampf \textperiodcentered\ Álvarez de
Zayas \textperiodcentered\ Hernández-Sampieri \textperiodcentered\ García-Córdoba \\
\addlinespace[3pt]
...
Iniciales con guion &
\texttt{Bathe K.-J.} \newline \texttt{Ryu H.-R.} &
\texttt{Bathe KJ} \newline \texttt{Ryu HR} &
\textcolor{ojo}{\textbf{Error corregible.}} Una línea en el preámbulo (§7). \\
\addlinespace[3pt]

Lugar de edición &
Cook 2002 y el manual de CSI salen sin lugar &
El lugar es obligatorio en libros y manuales &
\textcolor{ojo}{\textbf{Falta un dato.}} Leerlo de la portada de cada ejemplar. \\
\addlinespace[3pt]
...
% =====================================================================
\section{Las dos correcciones concretas}
...
Klaus-Jürgen sale \texttt{K.-J.} y Hee-Ryong sale \texttt{H.-R.} en vez de \texttt{KJ} y
\texttt{HR}. Los tres \texttt{\textbackslash renewcommand} que hoy hay en el preámbulo
(\texttt{bibinitperiod}, \texttt{bibinitdelim} y el que falta) los cubre de una sola vez la
opción \texttt{terseinits}:
...
Dos entradas no tienen \texttt{location} y la norma lo exige: \texttt{cook2002concepts} y
\texttt{csi2017sap2000}. El dato se lee de la portada o de la página de créditos del ejemplar
---no de la web--- y se agrega como \texttt{location = \{Ciudad, ST\}}.
