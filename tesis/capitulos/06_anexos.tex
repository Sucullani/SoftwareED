...
Desde \textsf{Archivo} $\rightarrow$ \textsf{Exportar} se genera la \textbf{Memoria de Cálculo} en PDF, un documento que reproduce el paso a paso simbólico y numérico del modelo concreto del usuario --funciones de forma evaluadas en los puntos de Gauss, Jacobiano, matrices $\mathbf{B}$ y $\mathbf{D}$, rigidez elemental, ensamblaje, partición de restricciones, resolución y recuperación de tensiones-- en dos estilos, educativo o directo. El mismo menú exporta el modelo a CSV/Excel para su edición masiva. Los proyectos se guardan en el formato nativo \texttt{.edufem}.
...
M2 & Proceso      & Jacobiano $\det\mathbf{J}(\xi,\eta)$: distorsión local, superficie 3D. & \texttt{Ctrl+2} \\
M3 & Proceso      & Matriz $\mathbf{B}$ (deformación): $\partial N/\partial x$ mediante $\mathbf{J}^{-1}$, con anclaje a puntos de Gauss. & \texttt{Ctrl+3} \\
M4 & Proceso      & Matriz constitutiva $\mathbf{D}(E,\nu,\text{caso})$ según el material del elemento. & \texttt{Ctrl+4} \\
M5 & Proceso      & Rigidez $\mathbf{k}_e$ y cuadratura de Gauss: la integral imposible se aproxima como suma en los puntos de Gauss. & \texttt{Ctrl+5} \\
M6 & Proceso      & Fuerzas equivalentes nodales: carga sobre arista y peso propio. & \texttt{Ctrl+6} \\
M7 & Proceso      & Ensamblaje de $\mathbf{K}$ y $\mathbf{F}$ con restricciones: vuelo Bézier y patrón de dispersión. & \texttt{Ctrl+7} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{longtable}{@{}>{\ttfamily}l l p{6.6cm} c@{}}
...
\addlinespace
no\_restraints & Error crítico & No hay ninguna restricción definida; el modelo conserva los modos de cuerpo rígido y la matriz de rigidez reducida $\mathbf{K}$ resulta singular. & No \\
\addlinespace
...
Ensambla $\mathbf{B}$ (de dimensión $3\times 2n$) a partir de las derivadas físicas de las funciones de forma, materializando la relación $\boldsymbol{\varepsilon} = \mathbf{B}\,\mathbf{u}$.
...
Relaciona tensiones y deformaciones, $\boldsymbol{\sigma} = \mathbf{D}\,\boldsymbol{\varepsilon}$, con las dos variantes de elasticidad plana.
...
\begin{table}[htbp]
\centering
\caption{Guía de reproducibilidad de la batería de verificación y validación: comando de ejecución, objeto de la prueba y resultados asociados en el Capítulo~4 y en este anexo.}
\label{tab:vyv-repro}
...
Validación del motor con los datos de ejemplo: resolución del sistema $\mathbf{K}\,\mathbf{u}=\mathbf{F}$, reacciones, tensiones nodales promedio y calidad de malla para los casos Q4 y Q9. &
...
\end{minipage}
\caption{Campos discretos $\mathbf{u}_h$ sobre la malla Q9 con $N=16$ (2178 GDL).}
...
\chapter{Memoria de Cálculo generada por el software}
\label{anexo:memoria}

Para ilustrar de principio a fin el resultado documental distintivo de EduFEM, se reproduce a continuación la \emph{Memoria de Cálculo} completa que el programa genera automáticamente para el ejemplo canónico de validación (cuadrado de nueve nodos, elemento Q4, tensión plana). El documento se obtiene desde \textsf{Archivo} $\rightarrow$ \textsf{Exportar} $\rightarrow$ \textsf{Memoria de Cálculo (PDF)}, en su estilo \emph{educativo}, y recorre el procedimiento del MEF paso a paso --funciones de forma evaluadas en los puntos de Gauss, Jacobiano, matrices $\mathbf{B}$ y $\mathbf{D}$, rigidez elemental e integración numérica, ensamblaje, partición de restricciones, resolución y recuperación de tensiones-- sobre el modelo concreto del usuario. Las páginas siguientes son el PDF tal como lo produce el software, sin edición posterior.

\includepdf[pages=-,scale=0.92,pagecommand={\thispagestyle{plain}}]{anexo_memoria_calculo.pdf}
