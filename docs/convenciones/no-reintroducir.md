...
| Un visor de PDF propio para la Teoría MEF (Toplevel que dibuja las páginas) y **PyMuPDF** (`fitz`) | PyMuPDF es **AGPL-3.0** o comercial: dentro del instalador dejaba el paquete entero sujeto a esa licencia, contra el MIT de EduFEM. Desde el 2026-09-25 la Teoría se abre con el visor del sistema, como la Memoria; `build.spec` excluye `fitz`/`pymupdf` y `test_distribucion` lo vigila | [MEM] |
| Un texto propio de la Teoría cuando falta `pdflatex` | Misma causa que la Memoria = misma salida: `show_pdflatex_missing_dialog(parent, documento=…)`, con botón de descarga | [MEM] |
| El hash del caché (`a3f2b9c1….pdf`) como nombre del PDF de teoría | El visor del sistema lo muestra en su barra de título y no le dice nada al alumno: el hash va en la carpeta, el título en el archivo | [MEM] |
...
| **M4 (D)** | El dial circular; el Entry numérico `ν =`; el botón "Reset al ν del material"; el título `ν = …` sobre la matriz; la regla de materiales dentro del probe; los nombres largos en el espectro; el caption "Efecto Poisson"; el rótulo "deformación exagerada"; las flechas ámbar de contracción; el semáforo de 4 colores del fill; **cualquier ancho del probe dependiente de ν**; las constantes muertas `_PROBE_VIS_SCALE`/`_CONTRACT_COLOR`; la línea de isotropía aparte; las flechas de ∇Nᵢ sobre el elemento; el **material de reemplazo sin elemento** (`_DEMO_E`/`_DEMO_NU` o el fallback a `materials[0]`: la D iba a ceros) y el ◆ «tu material» dibujado sin material | [EDU] |
| **M5** | El `GaussCoordReadout` tk; `_lbl_k_title`; el viewport de kₑ de 300 px; el `after(120)` sin cancelar; el triple mensaje de estado vacío; la kₑ, el `Σ n/n pg` y los PGs dorados del elemento **deseleccionado** (se vacían las contribuciones); `n_nodes = 4` fijo sin selección (un proyecto Q9 anunciaba «kₑ es 8×8 · Q4») | [EDU] |
| **M6** | Un `on_element_deselected`: M6 opera sobre las **cargas superficiales**, no sobre el elemento — su `on_element_selected` es un no-op deliberado | [EDU] |
